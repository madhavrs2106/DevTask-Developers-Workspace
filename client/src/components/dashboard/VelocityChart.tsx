...
const GRADIENT_ID = "velocityGradient";
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 8, right: 4, bottom: 0, left: -22 }} barCategoryGap="30%">
          ...
          <YAxis
            ...
            tickLine={false}
          />
          <Tooltip cursor={{ fill: "rgba(20,184,166,.06)" }} content={<VelocityTooltip />} />
          <Bar dataKey="completed" radius={[6, 6, 2, 2]} maxBarSize={30}>
            {data.map((entry, i) => (
              <Cell
                key={entry.label}
...
function VelocityTooltip({
  ...
  active?: boolean;
  payload?: { payload?: { label: string; completed: number } }[];
}) {
  ...
  return (
    <div className="card px-3 py-2 text-xs shadow-glow-sm">
      <p className="font-mono font-semibold text-white">
        {point.completed} <span className="font-sans text-ink-faint">tasks shipped</span>
      </p>
